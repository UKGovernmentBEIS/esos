package uk.gov.esos.api.workflow.request.flow.esos.actionplan.common.validation;
...
public abstract class RequestCreateActionPlanRelatedValidator implements RequestCreateByRequestValidator<ReportRelatedRequestCreateActionPayload> {
    ...
    public RequestCreateValidationResult validateAction(final Long accountId, ReportRelatedRequestCreateActionPayload payload, AppUser appUser) {
        ...
        if (!accountAuthorizationResourceService.hasUserScopeToAccountAndResourceSubType(appUser, accountId,
                Scope.REQUEST_CREATE, RequestCreateActionType.RE_INITIATE_ACTION_PLAN_P3.name())) {
            overallValidationResult.setValid(false);
