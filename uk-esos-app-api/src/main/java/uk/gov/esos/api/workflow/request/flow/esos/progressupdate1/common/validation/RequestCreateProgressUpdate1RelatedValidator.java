package uk.gov.esos.api.workflow.request.flow.esos.progressupdate1.common.validation;
...
public abstract class RequestCreateProgressUpdate1RelatedValidator implements RequestCreateByRequestValidator<ReportRelatedRequestCreateActionPayload> {
    ...
    public RequestCreateValidationResult validateAction(final Long accountId, ReportRelatedRequestCreateActionPayload payload, AppUser appUser) {
        ...
        if (!accountAuthorizationResourceService.hasUserScopeToAccountAndResourceSubType(appUser, accountId,
                Scope.REQUEST_CREATE, RequestCreateActionType.RE_INITIATE_PROGRESS_UPDATE_1_P3.name())) {
            overallValidationResult.setValid(false);
