package uk.gov.esos.api.workflow.request.flow.esos.progressupdate2.common.validation;
...
public abstract class RequestCreateProgressUpdate2RelatedValidator implements RequestCreateByRequestValidator<ReportRelatedRequestCreateActionPayload> {
    ...
    public RequestCreateValidationResult validateAction(final Long accountId, ReportRelatedRequestCreateActionPayload payload, AppUser appUser) {
        ...
        if (requestDetailsDTO.getRequestType() != this.getReferableRequestType()) {
            ...
            overallValidationResult.setReportedRequestTypes(Set.of(requestDetailsDTO.getRequestType()));
        }

        if (!accountAuthorizationResourceService.hasUserScopeToAccountAndResourceSubType(appUser, accountId,
                Scope.REQUEST_CREATE, RequestCreateActionType.RE_INITIATE_PROGRESS_UPDATE_2_P3.name())) {
            overallValidationResult.setValid(false);
