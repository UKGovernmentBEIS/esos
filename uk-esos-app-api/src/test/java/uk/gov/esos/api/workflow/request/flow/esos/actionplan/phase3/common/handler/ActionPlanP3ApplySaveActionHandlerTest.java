...
class ActionPlanP3ApplySaveActionHandlerTest {

    @InjectMocks
    private ActionPlanP3ApplySaveActionHandler handler;

    @Mock
    private RequestTaskService requestTaskService;

    @Mock
    private ActionPlanP3ApplyService actionPlanP3ApplyService;
    ...
    void process() {
        ...
        // Invoke
        handler.process(requestTaskId, RequestTaskActionType.ACTION_PLAN_P3_SAVE_APPLICATION_SUBMIT, appUser, taskActionPayload);

        // Verify
        verify(requestTaskService, times(1)).findTaskById(requestTaskId);
        verify(actionPlanP3ApplyService, times(1)).applySaveAction(taskActionPayload, requestTask);
    }

    @Test
    void getTypes() {
        assertThat(handler.getTypes()).containsExactly(RequestTaskActionType.ACTION_PLAN_P3_SAVE_APPLICATION_SUBMIT);
    }
