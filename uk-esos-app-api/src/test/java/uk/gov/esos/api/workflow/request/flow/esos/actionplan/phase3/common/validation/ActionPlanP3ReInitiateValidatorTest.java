...
class ActionPlanP3ReInitiateValidatorTest {

    @InjectMocks
    private ActionPlanP3ReInitiateValidator validator;
    ...
    @Test
    void getType() {
        assertThat(validator.getType()).isEqualTo(RequestCreateActionType.RE_INITIATE_ACTION_PLAN_P3);
    }
    ...
    void validateAction() {
        ...
        final String requestId = "AP000001-P3";
        ...
        final RequestDetailsDTO requestDetailsDTO = new RequestDetailsDTO(requestId, RequestType.ACTION_PLAN_P3,
                RequestStatus.COMPLETED, LocalDateTime.now(), ActionPlanP3RequestMetadata.builder().build());

        when(requestQueryService.findRequestDetailsByIdAndAccountId(requestId, accountId)).thenReturn(requestDetailsDTO);
        when(accountAuthorizationResourceService.hasUserScopeToAccountAndResourceSubType(appUser, accountId, Scope.REQUEST_CREATE, RequestCreateActionType.RE_INITIATE_ACTION_PLAN_P3.name()))
                .thenReturn(true);
        ...
        verify(accountAuthorizationResourceService, times(1))
                .hasUserScopeToAccountAndResourceSubType(appUser, accountId, Scope.REQUEST_CREATE, RequestCreateActionType.RE_INITIATE_ACTION_PLAN_P3.name());
    ...
    void validateRequestType() {
        final Long accountId = 1L;
        final RequestDetailsDTO requestDetailsDTO = new RequestDetailsDTO("AP000001-P3", RequestType.ACTION_PLAN_P3,
                RequestStatus.COMPLETED, LocalDateTime.now(), ActionPlanP3RequestMetadata.builder().build());

        when(requestQueryService.existsRequestByAccountAndTypeAndStatus(accountId, RequestType.PROGRESS_UPDATE_1_P3, RequestStatus.COMPLETED)).thenReturn(false);
    ...
    void validateRequestType_In_progress_not_valid() {
        final Long accountId = 1L;
        final RequestDetailsDTO requestDetailsDTO = new RequestDetailsDTO("AP000001-P3", RequestType.ACTION_PLAN_P3,
                RequestStatus.IN_PROGRESS, LocalDateTime.now(), ActionPlanP3RequestMetadata.builder().build());
    ...
    void validateRequestType_progressUpdate_exist_not_valid() {
        final Long accountId = 1L;
        final RequestDetailsDTO requestDetailsDTO = new RequestDetailsDTO("AP000001-P3", RequestType.NOTIFICATION_OF_COMPLIANCE_P3,
                RequestStatus.COMPLETED, LocalDateTime.now(), ActionPlanP3RequestMetadata.builder().build());

        when(requestQueryService.existsRequestByAccountAndTypeAndStatus(accountId, RequestType.PROGRESS_UPDATE_1_P3, RequestStatus.COMPLETED)).thenReturn(true);
    ...
    void getReferableRequestType() {
        assertThat(validator.getReferableRequestType()).isEqualTo(RequestType.ACTION_PLAN_P3);
    }
