...
#Update user session limits execution
CONFIGURE_SESSION_LIMITS_EXECUTION=$(curl -s -L -X PUT "$UPDATE_REALM_URL$UK_ESOS_REALM_NAME"/authentication/flows/"$ESOS_BROWSER"/executions \
-H 'Content-Type: application/json' \
-H "Authorization: Bearer $KEYCLOAK_ADMIN_ACCESS_TOKEN" \
--data-raw '{
    "id": "'"$USER_SESSION_LIMITS_ID"'",
